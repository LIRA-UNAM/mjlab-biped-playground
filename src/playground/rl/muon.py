"""Hybrid Muon optimizer and its PPO integration.

Ported from booster_mjlab. Muon orthogonalizes the update of every 2D weight
matrix, while Adam handles the remaining parameters (biases, the action std,
normalizer buffers, ...).
"""
  ...
  def __init__(
    ...
    *,
    ...
    self.muon = torch.optim.Muon(
      ...
      # Scale the orthogonalized update to Adam's RMS so both halves can share
      # one learning rate and the adaptive-KL schedule that drives it.
